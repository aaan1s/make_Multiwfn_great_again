...
! 		end where
	...
	! 		write(*,*) "Robost: 1=2>3"
			...
			call integratebasinmix(10,basinisoval)
		end if
...
subroutine integratebasinmix(itype,basinisoval)
use defvar
use util
use function
use basinintmod
...
! 			end do
			...
			if (cubmat(ix,iy,iz)>basinisoval) basinvdwvolp(iatt)=basinvdwvolp(iatt)+1
				...
				do ixref=1,nrefine
					do iyref=1,nrefine
						do izref=1,nrefine
							...
							rnowytmp=orgyref+(iyref-0.5D0)*dyref
							rnowztmp=orgzref+(izref-0.5D0)*dzref
							if (cubmat(ix,iy,iz)<=basinisoval) then !Only refine the boundary inside vdW surface
...
! 								write(*,*) irk4
							...
							if (cubmat(ix,iy,iz)>basinisoval) basinvdwvolp(iattref)=basinvdwvolp(iattref)+1D0/ndiv
...
if (itype==1.or.itype==2.or.itype==3) then !Integrate specific real space function(s)
	...
	if (ifuncint==1) then
		...
		if (ispecial==0) then
			...
			write(*,*) "Total result:"
			write(*,*) "    Atom       Basin     Shannon       Fisher      Steric ene  Vol(rho>basinisoval)"
			do iatm=0,ncenter
				do iatt=1,numrealatt
					if (att2atm(iatt)==iatm.and.iatm==0) then
