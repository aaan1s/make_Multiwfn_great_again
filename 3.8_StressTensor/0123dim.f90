...
do while(.true.)
    write(*,*) "-10 Return to main menu"
	write(*,*) "-228 ACTIVATE STRESS TENSOR RUNNNN (works on custom grid only)"
	write(*,*) "-328 CALC DIV of STRESS TENSOR  (works on custom grid only)"
	write(*,*) "-428 CALC sum of negative Stress Tensor eigenvalues (works on custom grid only)"
	write(*,*) "-2 Obtain deformation property"
	write(*,*) "-1 Obtain promolecule property"
	write(*,*) "0 Set custom operation"
...
500 continue
if (ifuncsel/=-228.and.ifuncsel/=-328.and.ifuncsel/=-428) then
	...
	do iextpt=1,numextpt !Calculate function value
		...
		extpt(iextpt,5)=stresstens(1,2)
		extpt(iextpt,6)=stresstens(1,3)
		extpt(iextpt,7)=stresstens(2,2)
		extpt(iextpt,8)=stresstens(2,3)
		extpt(iextpt,9)=stresstens(3,3)
	end do 
	!$OMP END PARALLEL DO

else if (ifuncsel==-328) then
	...
	do iextpt=1,numextpt !Calculate function value
		...
		extpt(iextpt,5)=divstress(2)
		extpt(iextpt,6)=divstress(3)
	end do 
	!$OMP END PARALLEL DO

else if (ifuncsel==-428) then
	!$OMP PARALLEL DO SHARED(extpt) PRIVATE(iextpt,divstress,stresseval,stressevect) schedule(dynamic) NUM_THREADS(nthreads)
	do iextpt=1,numextpt !Calculate function value
		call CalcStressTensor(3,extpt(iextpt,1),extpt(iextpt,2),extpt(iextpt,3),stresstens,divstress,stresseval,stressevect)
		do i=1,3
			if (stresseval(i)<0D0) then
				extpt(iextpt,4)=stresseval(i)
			end if
		end do		
	end do 
	!$OMP END PARALLEL DO
...
501	if (ncustommap/=0) then !Cycling will stop when all the file have been dealed
	...
	if (ifuncsel/=-228.and.ifuncsel/=-328) then
		do iextpt=1,numextpt
			write(10,"(3f13.7,E20.10)") extpt(iextpt,1:3),extpt(iextpt,4)
		end do
	else if (ifuncsel==-228) then
		do iextpt=1,numextpt
			write(10,"(3f13.7,6E20.10)") extpt(iextpt,1:3),extpt(iextpt,4:9)
		end do
	else if (ifuncsel==-328) then
		do iextpt=1,numextpt
			write(10,"(3f13.7,3E20.10)") extpt(iextpt,1:3),extpt(iextpt,4:6)
		end do
	end if
	close(10)
