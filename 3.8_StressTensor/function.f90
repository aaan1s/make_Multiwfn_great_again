...
integer ifunc
real*8 x,y,z
real*8 stresstens(3,3), divstress(3), stresseval(3),stressevect(3,3)
if (ifunc==1) then
	if (allocated(b)) then
...
else if (ifunc==-3281) then ! print a1 of divergence stress tensor 
	call CalcStressTensor(2,x,y,z,stresstens,divstress)
	calcfuncall=divstress(1)
else if (ifunc==-3282) then ! print a2 of divergence stress tensor 
	call CalcStressTensor(2,x,y,z,stresstens,divstress)
	calcfuncall=divstress(2)
else if (ifunc==-3283) then ! print a1 of divergence stress tensor 
	call CalcStressTensor(2,x,y,z,stresstens,divstress)
...
subroutine CalcStressTensor(runtype,x,y,z,stresstens,divstress,stresseval,stressevect)
	real*8 x,y,z,wfnval(nmo),wfngrad(3,nmo),wfnhess(3,3,nmo),wfntens3(3,3,3,nmo)
	real*8 stresstens(3,3), tmparr(3,3), temp
	real*8, optional :: divstress(3),stresseval(3),stressevect(3,3)
	integer runtype,N
	
	stresstens=0D0
	if (present(divstress)) divstress=0D0
	if (present(stresseval)) stresseval=0D0
	if (present(stressevect)) stressevect=0D0

	! v1.5 - without explicit use of basis GTFs - reduced number of calculations for minor speedup
	if (runtype==1) then
		call orbderv(4,1,nmo,x,y,z,wfnval,wfngrad,wfnhess)
	else
		call orbderv(5,1,nmo,x,y,z,wfnval,wfngrad,wfnhess,wfntens3)
	end if 
	
	do i=1,nmo
		stresstens(1,1)=stresstens(1,1)-1D0/2D0*(MOocc(i)*wfngrad(1,i)*wfngrad(1,i)-MOocc(i)*wfnhess(1,1,i)*wfnval(i))
		stresstens(2,2)=stresstens(2,2)-1D0/2D0*(MOocc(i)*wfngrad(2,i)*wfngrad(2,i)-MOocc(i)*wfnhess(2,2,i)*wfnval(i))
		stresstens(3,3)=stresstens(3,3)-1D0/2D0*(MOocc(i)*wfngrad(3,i)*wfngrad(3,i)-MOocc(i)*wfnhess(3,3,i)*wfnval(i))
		stresstens(1,2)=stresstens(1,2)-1D0/2D0*(MOocc(i)*wfngrad(1,i)*wfngrad(2,i)-MOocc(i)*wfnhess(1,2,i)*wfnval(i))
		stresstens(1,3)=stresstens(1,3)-1D0/2D0*(MOocc(i)*wfngrad(1,i)*wfngrad(3,i)-MOocc(i)*wfnhess(1,3,i)*wfnval(i))
		stresstens(2,3)=stresstens(2,3)-1D0/2D0*(MOocc(i)*wfngrad(2,i)*wfngrad(3,i)-MOocc(i)*wfnhess(2,3,i)*wfnval(i))
	end do
	stresstens(2,1)=stresstens(1,2)
	stresstens(3,1)=stresstens(1,3)
	stresstens(3,2)=stresstens(2,3)
	
	if (runtype==2) then
		do i=1,nmo
		!da11/dx
		divstress(1)=divstress(1)-1D0/2D0*(2D0*MOocc(i)*wfnhess(1,1,i)*wfngrad(1,i)-MOocc(i)*wfnhess(1,1,i)*wfngrad(1,i)-MOocc(i)*wfntens3(1,1,1,i)*wfnval(i))
		!da21/dy
		divstress(1)=divstress(1)-1D0/2D0*(MOocc(i)*wfnhess(1,2,i)*wfngrad(2,i)+MOocc(i)*wfngrad(1,i)*wfnhess(2,2,i)-MOocc(i)*wfnhess(1,2,i)*wfngrad(2,i)-MOocc(i)*wfntens3(1,2,2,i)*wfnval(i))
		!da31/dz
		divstress(1)=divstress(1)-1D0/2D0*(MOocc(i)*wfnhess(1,3,i)*wfngrad(3,i)+MOocc(i)*wfngrad(1,i)*wfnhess(3,3,i)-MOocc(i)*wfnhess(1,3,i)*wfngrad(3,i)-MOocc(i)*wfntens3(1,3,3,i)*wfnval(i))
		
		!da12/dx
		divstress(2)=divstress(2)-1D0/2D0*(MOocc(i)*wfnhess(1,1,i)*wfngrad(2,i)+MOocc(i)*wfngrad(1,i)*wfnhess(2,1,i)-MOocc(i)*wfnhess(1,2,i)*wfngrad(1,i)-MOocc(i)*wfntens3(1,2,1,i)*wfnval(i))
		!da22/dy
		divstress(2)=divstress(2)-1D0/2D0*(2D0*MOocc(i)*wfnhess(2,2,i)*wfngrad(2,i)-MOocc(i)*wfnhess(2,2,i)*wfngrad(2,i)-MOocc(i)*wfntens3(2,2,2,i)*wfnval(i))
		!da32/dz
		divstress(2)=divstress(2)-1D0/2D0*(MOocc(i)*wfnhess(2,3,i)*wfngrad(3,i)+MOocc(i)*wfngrad(2,i)*wfnhess(3,3,i)-MOocc(i)*wfnhess(2,3,i)*wfngrad(3,i)-MOocc(i)*wfntens3(2,3,3,i)*wfnval(i))
	
		!da13/dx
		divstress(3)=divstress(3)-1D0/2D0*(MOocc(i)*wfnhess(1,1,i)*wfngrad(3,i)+MOocc(i)*wfngrad(1,i)*wfnhess(3,1,i)-MOocc(i)*wfnhess(1,3,i)*wfngrad(1,i)-MOocc(i)*wfntens3(1,3,1,i)*wfnval(i))
		!da23/dy
		divstress(3)=divstress(3)-1D0/2D0*(MOocc(i)*wfnhess(2,2,i)*wfngrad(3,i)+MOocc(i)*wfngrad(2,i)*wfnhess(3,2,i)-MOocc(i)*wfnhess(2,3,i)*wfngrad(2,i)-MOocc(i)*wfntens3(2,3,2,i)*wfnval(i))
		!da33/dz
		divstress(3)=divstress(3)-1D0/2D0*(2D0*MOocc(i)*wfnhess(3,3,i)*wfngrad(3,i)-MOocc(i)*wfnhess(3,3,i)*wfngrad(3,i)-MOocc(i)*wfntens3(3,3,3,i)*wfnval(i))
		end do
	end if

	if (runtype==3) then
		tmparr(:,:)=stresstens(:,:)
		call diagsymat(tmparr,stressevect,stresseval,idiagok)
		if (idiagok/=0) write(*,*) "Note: Diagonization of Stress Tensor failed!"
		N=size(stresseval)
		do i=1,N-1
			do j=i+1,N
				if (stresseval(i)>stresseval(j)) then
					temp=stresseval(i)
					stresseval(i)=stresseval(j)
					stresseval(j)=temp
			
					tmparr(:,1)=stressevect(:,i)
					stressevect(:,i)=stressevect(:,j)
					stressevect(:,j)=tmparr(:,1)	
				end if
			end do
		end do
	end if
