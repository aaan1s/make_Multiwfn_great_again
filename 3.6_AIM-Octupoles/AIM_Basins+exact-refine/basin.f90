	...
	if (isel==-10) then
		...
		do ibas=ilow,ihigh
			if (ilow/=0) then
				...
				if (selectyn=='n') where (interbasgrid==0) cubmattmp=0
			else
...
xxxint(-1:numrealatt), xxyint(-1:numrealatt), xxzint(-1:numrealatt), xyyint(-1:numrealatt), xyzint(-1:numrealatt),& ! My octupole try
xzzint(-1:numrealatt), yyyint(-1:numrealatt), yyzint(-1:numrealatt), yzzint(-1:numrealatt), zzzint(-1:numrealatt),&
xr2int(-1:numrealatt), yr2int(-1:numrealatt), zr2int(-1:numrealatt)
...
xxxintp(-1:numrealatt), xxyintp(-1:numrealatt), xxzintp(-1:numrealatt), xyyintp(-1:numrealatt), xyzintp(-1:numrealatt),& ! My octupole try
xzzintp(-1:numrealatt), yyyintp(-1:numrealatt), yyzintp(-1:numrealatt), yzzintp(-1:numrealatt), zzzintp(-1:numrealatt),&
xr2intp(-1:numrealatt), yr2intp(-1:numrealatt), zr2intp(-1:numrealatt)
...
! 		radw=2*radx**5/dfloat(radpotAIM+1)/(1-radx)**7*parm**3 *4*pi
	...
	!$OMP xxxintp,xxyintp,xxzintp,xyyintp,xyzintp,xzzintp,yyyintp,yyzintp,yzzintp,zzzintp,xr2intp,yr2intp,zr2intp)&
...
! 		if (dist>trustrad(iatt)) cycle !Discrete separation between atomic-center and uniform integration
		...
		if (itype==1.or.itype==2.or.itype==3) then !Integrate a function
			...
			xxxintp(iatt)=xxxintp(iatt)+rx*rx*rx*tmpval ! My octupole try
			xxyintp(iatt)=xxyintp(iatt)+rx*rx*ry*tmpval
			xxzintp(iatt)=xxzintp(iatt)+rx*rx*rz*tmpval
			xyyintp(iatt)=xyyintp(iatt)+rx*ry*ry*tmpval
			xyzintp(iatt)=xyzintp(iatt)+rx*ry*rz*tmpval
			xzzintp(iatt)=xzzintp(iatt)+rx*rz*rz*tmpval
			yyyintp(iatt)=yyyintp(iatt)+ry*ry*ry*tmpval
			yyzintp(iatt)=yyzintp(iatt)+ry*ry*rz*tmpval
			yzzintp(iatt)=yzzintp(iatt)+ry*rz*rz*tmpval
			zzzintp(iatt)=zzzintp(iatt)+rz*rz*rz*tmpval
			
			xr2intp(iatt)=xr2intp(iatt)+rx*dist*dist*tmpval
			yr2intp(iatt)=yr2intp(iatt)+ry*dist*dist*tmpval
			zr2intp(iatt)=zr2intp(iatt)+rz*dist*dist*tmpval
	...
	if (itype==1.or.itype==2.or.itype==3) then
		...
		xxxint=xxxint+xxxintp
		xxyint=xxyint+xxyintp
		xxzint=xxzint+xxzintp
		xyyint=xyyint+xyyintp
		xyzint=xyzint+xyzintp
		xzzint=xzzint+xzzintp
		yyyint=yyyint+yyyintp
		yyzint=yyzint+yyzintp
		yzzint=yzzint+yzzintp
		zzzint=zzzint+zzzintp

		xr2int=xr2int+xr2intp
		yr2int=yr2int+yr2intp
		zr2int=zr2int+zr2intp
...
!$OMP eleintp,xintp,yintp,zintp,xxintp,yyintp,zzintp,xyintp,xzintp,yzintp,&
!$OMP xxxintp,xxyintp,xxzintp,xyyintp,xyzintp,xzzintp,yyyintp,yyzintp,yzzintp,zzzintp,xr2intp,yr2intp,zr2intp)& 
!$OMP shared(intval,basinvol,basinvdwvol,ifinish) NUM_THREADS(nthreads)
...
do iz=2,nz-1
	...
	do iy=2,ny-1
		...
		do ix=2,nx-1
			rnowx=xarr(ix)
			if ((itype==2.or.itype==3.or.itype==10).and.interbasgrid(ix,iy,iz)==.true.) cycle !If refine boundary grid at next stage, we don't calculate them at present stage
...
if (itype==1.or.itype==2.or.itype==3) then
	...
	xxxint=xxxint+xxxintp*dvol ! My Octupole try
	xxyint=xxyint+xxyintp*dvol
	xxzint=xxzint+xxzintp*dvol
	xyyint=xyyint+xyyintp*dvol
	xyzint=xyzint+xyzintp*dvol
	xzzint=xzzint+xzzintp*dvol
	yyyint=yyyint+yyyintp*dvol
	yyzint=yyzint+yyzintp*dvol
	yzzint=yzzint+yzzintp*dvol
	zzzint=zzzint+zzzintp*dvol

	xr2int=xr2int+xr2intp*dvol
	yr2int=yr2int+yr2intp*dvol
	zr2int=zr2int+zr2intp*dvol
...
if (itype==2.or.itype==3.or.itype==10) then
...
! 				end if
		...
		cycrk4:					do irk4=1,nrk4lim
									...
									if (itype==2.or.irk4==1.or.irk4==2.or.irk4>nrk4gradswitch.or.itype==10) then 
...
! 								write(*,*) irk4
							...
							if (ispecial==2.or.ifuncint==-1) then
								...
								if (itype==1.or.itype==2.or.itype==3) then									
									...
									eleintp(iattref)=eleintp(iattref)+tmpval*switchwei/ndiv
									xintp(iattref)=xintp(iattref)+rx*tmpval*switchwei/ndiv
									yintp(iattref)=yintp(iattref)+ry*tmpval*switchwei/ndiv
									zintp(iattref)=zintp(iattref)+rz*tmpval*switchwei/ndiv
									xxintp(iattref)=xxintp(iattref)+rx*rx*tmpval*switchwei/ndiv
									yyintp(iattref)=yyintp(iattref)+ry*ry*tmpval*switchwei/ndiv
									zzintp(iattref)=zzintp(iattref)+rz*rz*tmpval*switchwei/ndiv
									xyintp(iattref)=xyintp(iattref)+rx*ry*tmpval*switchwei/ndiv
									yzintp(iattref)=yzintp(iattref)+ry*rz*tmpval*switchwei/ndiv
									xzintp(iattref)=xzintp(iattref)+rx*rz*tmpval*switchwei/ndiv
									xxxintp(iattref)=xxxintp(iattref)+rx*rx*rx*tmpval*switchwei/ndiv ! My octupole try
									xxyintp(iattref)=xxyintp(iattref)+rx*rx*ry*tmpval*switchwei/ndiv
									xxzintp(iattref)=xxzintp(iattref)+rx*rx*rz*tmpval*switchwei/ndiv
									xyyintp(iattref)=xyyintp(iattref)+rx*ry*ry*tmpval*switchwei/ndiv
									xyzintp(iattref)=xyzintp(iattref)+rx*ry*rz*tmpval*switchwei/ndiv
									xzzintp(iattref)=xzzintp(iattref)+rx*rz*rz*tmpval*switchwei/ndiv
									yyyintp(iattref)=yyyintp(iattref)+ry*ry*ry*tmpval*switchwei/ndiv
									yyzintp(iattref)=yyzintp(iattref)+ry*ry*rz*tmpval*switchwei/ndiv
									yzzintp(iattref)=yzzintp(iattref)+ry*rz*rz*tmpval*switchwei/ndiv
									zzzintp(iattref)=zzzintp(iattref)+rz*rz*rz*tmpval*switchwei/ndiv
									xr2intp(iattref)=xr2intp(iattref)+rx*dist*dist*tmpval*switchwei/ndiv
									yr2intp(iattref)=yr2intp(iattref)+ry*dist*dist*tmpval*switchwei/ndiv
									zr2intp(iattref)=zr2intp(iattref)+rz*dist*dist*tmpval*switchwei/ndiv
								end if
...
101	eleinttot=0D0
	...
	do iatm=0,ncenter
		do iatt=1,numrealatt
			...
			write(ioutid,"(' Basin electric quadrupole moment (Cartesian non-traceless form):')")
			write(ioutid,"(' QXX=',f12.6,'  QXY=',f12.6,'  QXZ=',f12.6)") -xxint(iatt),-xyint(iatt),-xzint(iatt)
			write(ioutid,"(' QYX=',f12.6,'  QYY=',f12.6,'  QYZ=',f12.6)") -xyint(iatt),-yyint(iatt),-yzint(iatt)
			write(ioutid,"(' QZX=',f12.6,'  QZY=',f12.6,'  QZZ=',f12.6)") -xzint(iatt),-yzint(iatt),-zzint(iatt)

			write(ioutid,"(' Basin electric octupole moment (Cartesian form):')")
			OXXX=-(15*xxxint(iatt)-3*xr2int(iatt)-3*xr2int(iatt)-3*xr2int(iatt))
			OXXY=-(15*xxyint(iatt)-3*yr2int(iatt))
			OXXZ=-(15*xxzint(iatt)-3*zr2int(iatt))
			OXYY=-(15*xyyint(iatt)-3*xr2int(iatt))
			OXYZ=-15*xyzint(iatt)
			OXZZ=-(15*xzzint(iatt)-3*xr2int(iatt))
			OYYY=-(15*yyyint(iatt)-3*yr2int(iatt)-3*yr2int(iatt)-3*yr2int(iatt))
			OYYZ=-(15*yyzint(iatt)-3*zr2int(iatt))
			OYZZ=-(15*yzzint(iatt)-3*yr2int(iatt))
			OZZZ=-(15*zzzint(iatt)-3*zr2int(iatt)-3*zr2int(iatt)-3*zr2int(iatt))

			write(ioutid,"(' QXXX=',f12.6,'  QXXY=',f12.6,'  QXXZ=',f12.6)") OXXX,OXXY,OXXZ
			write(ioutid,"(' QXYX=',f12.6,'  QXYY=',f12.6,'  QXYZ=',f12.6)") OXXY,OXYY,OXYZ
			write(ioutid,"(' QXZX=',f12.6,'  QXZY=',f12.6,'  QXZZ=',f12.6)") OXXZ,OXYZ,OXZZ
		
			write(ioutid,"(' QYXX=',f12.6,'  QYXY=',f12.6,'  QYXZ=',f12.6)") OXXY,OXYY,OXYZ
			write(ioutid,"(' QYYX=',f12.6,'  QYYY=',f12.6,'  QYYZ=',f12.6)") OXYY,OYYY,OYYZ
			write(ioutid,"(' QYZX=',f12.6,'  QYZY=',f12.6,'  QYZZ=',f12.6)") OXYZ,OYYZ,OYZZ			

			write(ioutid,"(' QZXX=',f12.6,'  QZXY=',f12.6,'  QZXZ=',f12.6)") OXXZ,OXYZ,OXZZ
			write(ioutid,"(' QZYX=',f12.6,'  QZYY=',f12.6,'  QZYZ=',f12.6)") OXYZ,OYYZ,OYZZ
			write(ioutid,"(' QZZX=',f12.6,'  QZZY=',f12.6,'  QZZZ=',f12.6)") OXZZ,OYZZ,OZZZ


			write(ioutid,"(' Basin electric octupole moment (Cartesian non-traceless form):')")
			write(ioutid,"(' QXXX=',f12.6,'  QXXY=',f12.6,'  QXXZ=',f12.6)") -xxxint(iatt),-xxyint(iatt),-xxzint(iatt)
			write(ioutid,"(' QXYX=',f12.6,'  QXYY=',f12.6,'  QXYZ=',f12.6)") -xxyint(iatt),-xyyint(iatt),-xyzint(iatt)
			write(ioutid,"(' QXZX=',f12.6,'  QXZY=',f12.6,'  QXZZ=',f12.6)") -xxzint(iatt),-xyzint(iatt),-xzzint(iatt)
		
			write(ioutid,"(' QYXX=',f12.6,'  QYXY=',f12.6,'  QYXZ=',f12.6)") -xxyint(iatt),-xyyint(iatt),-xyzint(iatt)
			write(ioutid,"(' QYYX=',f12.6,'  QYYY=',f12.6,'  QYYZ=',f12.6)") -xyyint(iatt),-yyyint(iatt),-yyzint(iatt)
			write(ioutid,"(' QYZX=',f12.6,'  QYZY=',f12.6,'  QYZZ=',f12.6)") -xyzint(iatt),-yyzint(iatt),-yzzint(iatt)

			write(ioutid,"(' QZXX=',f12.6,'  QZXY=',f12.6,'  QZXZ=',f12.6)") -xxzint(iatt),-xyzint(iatt),-xzzint(iatt)
			write(ioutid,"(' QZYX=',f12.6,'  QZYY=',f12.6,'  QZYZ=',f12.6)") -xyzint(iatt),-yyzint(iatt),-yzzint(iatt)
			write(ioutid,"(' QZZX=',f12.6,'  QZZY=',f12.6,'  QZZZ=',f12.6)") -xzzint(iatt),-yzzint(iatt),-zzzint(iatt)
